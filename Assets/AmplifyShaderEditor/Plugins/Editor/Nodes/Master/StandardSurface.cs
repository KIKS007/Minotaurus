				...
				for ( int i = 0; i < sortedPorts.Count; i++ )
				{
					if ( sortedPorts[ i ].IsConnected )
					{
						...
						else if ( sortedPorts[ i ].Name.Equals( AlphaStr ) && m_currentLightModel == StandardShaderLightModel.CustomLighting )
						{
							...
							customLightingAlphaCode = node.GetValueFromOutputStr( connection.PortId, WirePortDataType.FLOAT, ref UIUtils.CurrentDataCollector, false );
							customLightingInstructions = UIUtils.CurrentDataCollector.CustomOutput;
				...
				OpenSubShaderBody( ref ShaderBody );
				{
						...
						OpenCGProgram( ref ShaderBody );
					{
						...
						if ( m_currentLightModel == StandardShaderLightModel.CustomLighting )
						{
							...
							if( UIUtils.CurrentDataCollector.UsingLightAttenuation )
							{
							...
							//if( UIUtils.CurrentDataCollector.dirtyc )
